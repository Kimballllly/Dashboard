...
app1.secret_key = 'your_secret_key_here'
app1.permanent_session_lifetime = timedelta(days=7)
...
@app1.route('/generate_report', methods=['GET'])
def generate_report():
    ...
    logo_path = os.path.join(app1.static_folder, "logo.jpg")
    header_table_data = []
...
@app1.route('/update_prices', methods=['POST'])
def update_prices():
    if 'admin_id' not in session:
...
@app1.route('/update_remaining_paper', methods=['POST'])
...
if __name__ == '__main__':
    app1.run(debug=True, host='127.0.0.1', port=5000)
